...
- Iteration 3 — skin: the layered north façade section (`js/facade.js`), the roof plan with beds on the beam strips, pergola, a service strip with one place per unit, the east-facing skylight and vent; cooling's place reserved, device still open.
- Exploded 3D shows every floor as a LAYOUT (plan/volume toggle, flat room fills and labels, void openings, top view from the south) and, below it, every floor separately as a plan grid.
- PDF links open for viewing in a new tab instead of forcing a download.
